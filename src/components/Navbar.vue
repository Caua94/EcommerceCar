    ...
    >
      <div class="max-w-7xl px-6 py-4 flex items-center h-full justify-between md:justify-start">
        ...
        <ul class="hidden md:flex ml-50 gap-15 font-bold text-2xl items-center">
          ...
          <router-link class="hover:text-amber-300 transition-colors" to="/seeMore">Create</router-link>
        </ul>
        <button
          @click="openProducts"
          ...
          <li class="relative group">
            ...
            <ul class="fixed left-full top-0 w-[30rem] h-screen bg-stone-900 shadow-lg p-6 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300">
              ...
              <li v-for="brand in brands" :key="brand.id">
                <router-link :to="`/shop/brand/${brand.nome}`" class="block py-1 hover:text-amber-300">{{ brand.nome }}</router-link>
              </li>
            </ul>
          </li>
          ...
          <li class="relative group">
            ...
            <ul class="fixed left-full top-0 w-[30rem] h-screen bg-stone-900 shadow-lg p-6 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300">
              ...
              <li v-for="category in categories" :key="category.id">
                <router-link :to="`/shop/category/${category.nome}`" class="block py-1 hover:text-amber-300">{{ category.nome }}</router-link>
              </li>
            </ul>
          </li>
        </ul>
